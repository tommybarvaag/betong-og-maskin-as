...
// plus a fixed bottom action bar. Breakpoint matches the prototype's 880px isMobile switch.
export function Header({ info }: { info: Info }) {
  ...
  return (
    <>
      ...
      <div className="fixed inset-x-0 bottom-0 z-40 flex gap-2.5 border-t border-white/10 bg-[#0f1320]/92 p-3 backdrop-blur-[12px] min-[880px]:hidden">
        <a
          href={tel}
          className={cn(
            buttonVariants(),
            "h-[52px] flex-1 gap-2.5 rounded-[9px] text-base",
          )}
        >
          <Phone className="size-[18px]" />
          Ring oss
        ...
        <a
          ...
          className={cn(
            buttonVariants({ variant: "glass" }),
            "bg-elevated h-[52px] flex-1 gap-2.5 rounded-[9px] border-white/[0.18] text-[15px]",
          )}
        >
          <Mail className="size-[18px]" strokeWidth={1.8} />
          E-post
